...
def generate_page(from_path, template_path, dest_path):
    ...
    changed_title = template.replace("{{ Title }}", title)
    index = changed_title.replace("{{ Content }}", html_string)
...
def generat_page_recursive(dir_path_content, template_path, dir_dest_path):
    ...
    for dir in os.listdir(abs_content_path):
        ...
        if os.path.isfile(joined_content_path):
            ...
            changed_title = template.replace("{{ Title }}", title)
            index = changed_title.replace("{{ Content }}", html_string)
            changed_joined_dest_path = joined_dest_path.replace(".md", ".html")

            with open(changed_joined_dest_path, "w") as file:
                file.write(index)
